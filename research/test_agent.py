...
def test_multimodal():
    for model in models:
        ...
        prompt = Prompt(
            ...
            documents=[os.path.join("docs", file) for file in files]
        )
        agent = AIAgent(model)
        try:
...
async def test_streaming():
    ...
    tool_calls = []
    max_length = 40
    async for chunk in agent.stream("whats the weather in kuala lumpur today?", stream_mode="messages"):
        ...
        if chunk[0] == "structured_response" or chunk[0] == "response":
            print(chunk[1])
        chunks.append(chunk)
...
async def test_loop_agent():
    def stop_or_proceed(inputs: dict) -> bool | tuple[bool, str]:
        return "3" in inputs["response"].content, f"Return only the number {inputs["iteration"]}, nothing else"
    agent = AIAgent(model="mistral-small")
    loop = LoopAgent(agent, stop_or_proceed, 5)
